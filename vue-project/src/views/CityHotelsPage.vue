<template>
  <div>
    ...
    <div>
      <h2 class="text-3xl font-bold mb-4">{{city[0].toUpperCase() + city.substring(1)}} Hotels</h2>
            ...
            <RouterLink :to="'/hotel/'+ hotel.itemId">
              ...
              <div class="flex space-x-3 items-center mb-2">
                <div class="text-white p-1 rounded-lg bg-violet-600 w-[40px] items-center justify-center flex">
                  <p>{{hotel.rating}}</p>
                  <img src="http://localhost:5173/src/assets/img/star.svg" class="h-3.5 ml-1"/>
                </div>
                <div class="text-sm">{{hotel.num_reviews}} reviews</div>
              </div>
              <div class="flex justify-end text-gray-700 mb-2 text-sm">Starting Price:
                <span class="font-semibold"> €{{ hotel.avg_price }} per night</span>
              </div>
            </RouterLink>
          </div>
        </template>
...
const searchHotels = async() => {
  await axios.get('http://127.0.0.1:8000/api/get-hotels-by-city', {params: {
      city: city.value,
    }})
      .then((response) => {
